  ...
  useEffect(() => {
    const fetchNewReleases = async () => {
      try {
        ...
        setNewReleases(response.data.data.slice(0, 6));
      } catch (error) {
    ...
    <StyledContainer fluid>
      ...
      <Row className="px-3">
        {radioEpisodes.map((episode, index) => (
          <Col xs={4} key={index}>
            <Card>
              <SquareCardImage src={episode.image} />
      ...
      <Row className="px-3 ">
        {newReleases.map((release) => (
          <Col xs={4} sm={3} md={2} key={release.id}>
            {" "}
            {}
            <Card>
              <SquareCardImage src={release.album.cover_medium} />
